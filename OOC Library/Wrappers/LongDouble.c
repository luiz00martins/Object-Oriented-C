...
define_method(ctor){
    paramOptional(long double, val, 0);

    self->data = val;

    returning(LongDouble, self);
}
...
define_method(scan){
    ...
    char c;
    scanf("%100s%c", arr, &c);
    self->data = strtold(arr, NULL);

    returning();
}

define_method(equals){
    param(LongDouble, obj);
...
define_method(get){
    returning(long double, self->data);
}
